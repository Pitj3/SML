...
namespace sml
{
        ...
        public:
            ...
            inline bool operator == (const mat3& other) const
            {
                ...
                if constexpr (std::is_same<T, f64>::value)
                {
                    ...
                    for (s32 i = 0; i < 3; i++)
                    {
                        __m128d me = _mm_load_pd(&m00 + (2 * i + 0));
                        __m128d me1 = _mm_load_pd(&m00 + (2 * i + 2));
                        __m128d ot = _mm_load_pd(&other.m00 + (2 * i + 0));
                        __m128d ot1 = _mm_load_pd(&other.m00 + (2 * i + 2));

                        m128 cmp = { _mm_cmpeq_pd(me, ot) };
                        m128 cmp1 = { _mm_cmpeq_pd(me1, ot1) };
                        result &= _mm_testc_si128(cmp.i, cmp.i);
                        result &= _mm_testc_si128(cmp1.i, cmp1.i);
                    }
            ...
            inline bool operator != (const mat3& other) const
            {
                ...
                if constexpr (std::is_same<T, f64>::value)
                {
                    ...
                    for (s32 i = 0; i < 3; i++)
                    {
                        ...
                        m128 cmp1 = { _mm_cmpneq_pd(me1, ot1) };
                        result &= _mm_testc_si128(cmp.i, cmp.i);
                        result &= _mm_testc_si128(cmp1.i, cmp1.i);
                    }
    ...
    vec3<T> operator * (const mat3<T>& lhs, const vec3<T>& rhs)
    {
        ...
        if constexpr (std::is_same<T, f64>::value)
        {
            ...
            _mm256_store_pd(res.v, _mm256_add_pd(_mm256_mul_pd(x, c0), _mm256_add_pd(_mm256_mul_pd(y, c1), _mm256_mul_pd(z, c2))));
